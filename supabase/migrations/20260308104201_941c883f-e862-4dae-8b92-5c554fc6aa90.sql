...
DECLARE
  ...
  v_last_night date;
BEGIN
  -- Acquire an advisory lock so concurrent calls serialize
  ...
  LOOP
    ...
    -- This room is available — book it
    v_subtotal := v_room.price_per_night * v_nights;
    v_tax := round(v_subtotal * 0.18);
    v_total := v_subtotal + v_tax;
    v_id := gen_random_uuid();
    v_booking_id := 'GC' || to_char(now(), 'YYYYMMDD') || '-' || substring(v_id::text, 1, 8);
    ...
    VALUES (v_id, v_booking_id, v_room.id, p_room_type::room_type, p_check_in, p_check_out,
      v_nights, v_total, 'pending', p_full_name, p_mobile, p_email, p_adults, p_children,
      p_special_requests, p_user_id, 'unpaid');

    -- Create invoice
    v_invoice_id := gen_random_uuid();
    v_invoice_number := 'INV-' || to_char(now(), 'YYYYMMDD') || '-' || substring(v_invoice_id::text, 1, 6);
